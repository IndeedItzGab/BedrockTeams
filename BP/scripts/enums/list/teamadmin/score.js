...
enumAdminRegistry(messages.command.score, async (origin, firstArgs, secondArgs, thirdArgs) => {
  ...
  if(!firstArgs || !secondArgs || !thirdArgs) return player.sendMessage(messageSyntax(`/teamadmin ${messages.command.score} ${messages.helpArg.admin.score}`))
  ...
  switch(firstArgs) {
    case "set":
      team.score = Math.max(setting.teams["minScore"], thirdArgs);
      break;
    case "add":
