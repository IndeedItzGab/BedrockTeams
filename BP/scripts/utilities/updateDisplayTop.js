  ...
  if(setting.teams["displayTopTeams"]) {
    system.run(async () => {   
      ...
      for(const team of teams) {
        objectiveData.setScore(messages.displayTopTeams.body.replace("{0}", team.color).replace("{1}", team.name).replace("{2}", setting.teams["levels"].filter(d => !d?.price || d.price <= team.score).length), team.score)
      }
